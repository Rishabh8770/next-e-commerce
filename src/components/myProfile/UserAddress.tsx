...
const UserAddress = ({
  maxShippingAddressToShow = null,
  maxBillingAddressToShow = null,
  isProfilePage = false,
}: UserAddressProp) => {
  ...
  const shippingAddressToDisplay = maxShippingAddressToShow
    ? addresses.shippingAddresses.slice(0, maxShippingAddressToShow)
    : addresses.shippingAddresses;
  const billingAddressToDisplay = maxBillingAddressToShow
    ? addresses.billingAddresses.slice(0, maxBillingAddressToShow)
        ...
        <div className="flex md:flex-row flex-col md:space-x-10">
          <div className="md:w-1/2 w-full border rounded-lg p-5 bg-white md:mb-0 mb-5">
            ...
            {shippingAddressToDisplay.length > 0 ? (
              ...
              <p className="mt-4">shipping {ERROR_MESSAGE.noAddress}</p>
            )}
          </div>
          <div className="md:w-1/2 w-full border rounded-lg p-5 bg-white">
            ...
            {billingAddressToDisplay.length > 0 ? (
              ...
              <p>Billing {ERROR_MESSAGE.noAddress}</p>
            )}
          </div>
        </div>
        <div
